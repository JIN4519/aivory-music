...
export function SpotifyResults({ query, onArtistClick }: SpotifyResultsProps) {
  ...
  useEffect(() => {
    ...
    const fetchSpotifyResults = async () => {
      setLoading(true);
      const results = await searchSpotifyArtists(query, 20);
      
      // 중복된 아티스트 제거 (같은 ID를 가진 아티스트)
    ...
    const debounce = setTimeout(fetchSpotifyResults, 300);
  ...
  return (
    <div className="mb-12">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-white text-2xl">아티스트 검색 결과 ({artists.length}개)</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-[#131739] rounded-2xl border border-[#060629]">
          ...
          <p className="text-[#747798]">검색 결과가 없습니다</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
